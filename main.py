...
def train():
    ...
    with trange(FLAGS.total_steps, dynamic_ncols=True) as pbar:
        for step in pbar:
            ...
            # dummy sampling to test checkpoint saving
            if step == 20000:
                checkpoint = torch.load(os.path.join(FLAGS.logdir, 'ckpt.pt'))
                temp_model = UNet(
                    T=FLAGS.T, ch=FLAGS.ch, ch_mult=FLAGS.ch_mult, attn=FLAGS.attn,
                    num_res_blocks=FLAGS.num_res_blocks, dropout=FLAGS.dropout
                )
                temp_model.load_state_dict(checkpoint['ema_model'])
                temp_model.to(device)
                temp_model.eval()

                # Run the sampler
                temp_sampler = GaussianDiffusionSampler(
                    temp_model, FLAGS.beta_1, FLAGS.beta_T, FLAGS.T, FLAGS.img_size,
                    FLAGS.mean_type, FLAGS.var_type
                ).to(device)
                with torch.no_grad():
                    t_x_T = torch.randn(FLAGS.sample_size, 3, FLAGS.img_size, FLAGS.img_size).to(device)
                    sample_images = temp_sampler(t_x_T)

                # Save the sample images
                save_image((make_grid(sample_images) + 1) / 2, os.path.join(FLAGS.logdir, 'mysample_ema20000.png'))

                # After using the temp_model and sampler
                del temp_model
                del temp_sampler
...
def eval():
    ...
    #     grid = make_grid(x_0)
    #     path = os.path.join(
    #         FLAGS.logdir, 'my_samples', 'mysample.png')
    #     save_image(grid, path)
    # model.train()
    ...
    # with torch.no_grad():
    #     #x_0 = ema_sampler(x_T).cpu()
    #     x_0 = ema_sampler(x_T)
    #     grid = (make_grid(x_0) + 1) / 2
    #     #grid = make_grid(x_0)
    #     path = os.path.join(
    #         FLAGS.logdir, 'my_samples', 'mysample_ema.png')
    #     save_image(grid, path)
    # model.train()
    # (IS, IS_std), FID, samples = evaluate(sampler, model)
    ...
    # Save the sample images
    save_image((make_grid(sample_images) + 1) / 2, os.path.join(FLAGS.logdir, 'my_samples', 'mysample_ema.png'))
